...
@property (strong, nonatomic) ImageGalleryView *imageGallery;
@property (assign, nonatomic) BOOL isHiddenNavBar;
@property (assign, nonatomic) CGPoint startContentOffset;
...
- (void)configureView {
  ...
  self.costCalyanLabel.text = [NSString stringWithFormat:@"%@ руб",self.placeDetails.costCalian];
...
- (void)addImageGallery {
  NSString *imageUrlString = [NSString stringWithFormat:@"%@%@",SERVER_BASE_URL,_placeDetails.mainImage];
  self.imageGallery.images = @[imageUrlString, imageUrlString, imageUrlString];
  
  ImageGalleryViewController *imageGalleryViewController = [[ImageGalleryViewController alloc] initWithImages:self.imageGallery.images];
  imageGalleryViewController.receiveCurrentState = ^void (NSInteger currentIndex) {
    self.imageGallery.currentIndex = currentIndex;
  };
  
  __block PlaceDetailsViewController *weakSelf = self;
  [self.imageGallery setImageSetFullScreen:^(NSInteger index) {
    [weakSelf performSegueWithIdentifier:@"ImageFullscreenSegue" sender:nil];
  }];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  if ([segue.identifier isEqualToString:@"ImageFullscreenSegue"]) {
    ((ImageGalleryViewController*)segue.destinationViewController).currentIndex = self.imageGallery.currentIndex;
  }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  if((differenceFromStart) < 0)
  {
    ...
    if(_isHiddenNavBar && scrollView.contentOffset.y >=  self.imageGalleryContainer.frame.size.height- self.navigationController.navigationBar.frame.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)
    {
    ...
    if (!_isHiddenNavBar && scrollView.contentOffset.y <  self.imageGalleryContainer.frame.size.height- self.navigationController.navigationBar.frame.size.height) {
      [UIView animateWithDuration:0.1 animations:^{
